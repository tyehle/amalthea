...
def split_update(P, m, T0, Tcurr, S, c, f, g):
    ...
    P_sub = [randint(1, 2) for _ in g_sub.vs]
    while T > Tcurr:
        # Propose fS**2 individual node movements
        for i in range(int(f * S**2)):
            Pnew_sub = list(P_sub)
...
def conn_graph(g_given):
    ...
    for z in range(len(g.vs['zipcode'])):
        ...
        zip_list.append(shapely.geometry.asShape(_geometry.find_one({'zip': g.vs['zipcode'][z]})['geometry']))
...
if __name__ == '__main__':
    g = igraph.Graph.Read_GraphML('data/baltimore/distance/2.4/zip/networks/17dec2010.graphml')
